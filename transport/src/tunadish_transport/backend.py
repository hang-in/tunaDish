...
class TunadishBackend:
    ...
    async def _handle_project_context(self, params: dict[str, Any], runtime: TransportRuntime, transport: TunadishTransport):
        ...
        if (ctx is None or not ctx.project) and conv_id != "__rpc__" and not conv_id.startswith("branch:"):
            from tunapi.context import RunContext as _BindRC
        ...
        if not model and engine:
            try:
                ...
                if runner_model:
                    ...
                    # run_options에서 모델 확인
                    from tunapi.runners.run_options import get_run_options
        ...
        from tunapi.context import RunContext as _RC
        project_path = None
        try:
        ...
        if project_path:
            try:
                ...
                logger.debug("git rev-parse rc=%s stdout=%r stderr=%r", result_git.returncode, result_git.stdout.strip(), result_git.stderr.strip())
        ...
        try:
            for eid in runtime.available_engine_ids():
                from tunapi.engine_models import KNOWN_MODELS
